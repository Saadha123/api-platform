"""
Azure OpenAI proxy tests – uses the openai AzureOpenAI SDK.

Each enabled proxy with provider_type=azure_openai in config.yaml gets its own
parametrised run.  Tests mirror test_azure_openai_provider.py exactly.
"""
...
_PROXIES = enabled_proxies_by_type(_cfg, "azure_openai")
...
_skip = pytest.mark.skipif(
    ...
    reason="No Azure OpenAI proxies enabled in config.yaml",
)
...
def _client(proxy: Dict[str, Any]) -> AzureOpenAI:
    ...
    return AzureOpenAI(
        ...
        api_version=proxy.get("api_version", "2024-06-01"),
        default_headers={"X-API-Key": proxy["api_key"]},
